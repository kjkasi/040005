...
//	new->timeout.expires = jiffies + (TRIGGER_TIMEOUT * HZ); 
	new->timeout.expires = (trig->timeout.expires);
/*NETGEAR Changes End */
...
/*   NETGEAR Changes Start */
 		...
 		trigger_refresh(found, ((info->timeout) * HZ)); 
/* NETGEAR Changes End */
        /* In order to allow multiple hosts use the same port range, we update 
           the 'saddr' after previous trigger has a reply connection. */ 
        if (found->reply) {
            found->srcip = iph->saddr;
		return NF_ACCEPT;        /* We don't block any packet. */
		}
		}
	}
...
/*   NETGEAR Changes Start */
//		trig.timeout.expires=info->timeout;
		trig.timeout.expires = jiffies + (info->timeout * HZ); 
/*   NETGEAR Changes End */
...
/*   NETGEAR Changes End */
    ...
    return NF_ACCEPT;        /* Our job is the interception. */ 
} 
...
static struct xt_target ipt_trigger_reg __read_mostly = {  
        .name           = "TRIGGER", 
	.family         = NFPROTO_IPV4,
