...
//_TXPWR_REDEFINE in MP Tool, 3 Groups: 36-99 100-148 149-165
		...
		//panic_printk("PG file_format : %s\n", ((file_format == THREE_COLUMN) ? "THREE_COLUMN" : ((file_format == FIVE_COLUMN) ? "FIVE_COLUMN" : "SEVEN_COLUMN")));
...
void RTLWIFINIC_GPIO_config(unsigned int gpio_num, unsigned int direction)
{
	...
	//#+ WNC_JH, enable GPIO 8 - 15 control on /proc/wlan0/gpio_ctrl
	if ((GET_CHIP_VER(priv) == VERSION_8192E) || (GET_CHIP_VER(priv) == VERSION_8881A)) {
...
/* Netgear add start */
EXPORT_SYMBOL(RTLWIFINIC_GPIO_config);
EXPORT_SYMBOL(RTLWIFINIC_GPIO_write);
/* Netgear add end */
