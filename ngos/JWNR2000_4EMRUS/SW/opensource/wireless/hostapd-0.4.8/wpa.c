...
/* Increase the number of retries to improve the chances of
   successful re-keying
*/
static const u32 dot11RSNAConfigGroupUpdateTimeOut = 1200; /* ms */
static const u32 dot11RSNAConfigGroupUpdateCount = 5;
static const u32 dot11RSNAConfigPairwiseUpdateTimeOut = 1200; /* ms */
static const u32 dot11RSNAConfigPairwiseUpdateCount = 5;
		  ...
		  wpa_event event)
{
	...
	case WPA_REAUTH_EAPOL:
        /*
         * Decrement GKeyDoneStations if the state machine was
         * in Groupe Key Update state before the reauthentication
        */
        if (sm->GUpdateStationKeys) {
            hapd->wpa_auth->GKeyDoneStations--;
            sm->GUpdateStationKeys = FALSE;
        }
        sm->wpa_ptk_group_state=WPA_PTK_GROUP_IDLE;
		sm->ReAuthenticationRequest = TRUE;
		break;
	}
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	...
	if (sm->sta->wpa == WPA_VERSION_WPA &&
        ...
        if(sm->sta->wpa == WPA_VERSION_WPA2 &&
        (sm->hapd->conf->wpa & HOSTAPD_WPA_VERSION_WPA2) &&
        (sm->hapd->conf->wpa & HOSTAPD_WPA_VERSION_WPA))
        {
            /* WPA2-only STA, use only RSN IE */
            wpa_ie_len = wpa_ie[1]+2;
        }
    }
	hostapd_logger(sm->hapd, sm->sta->addr, HOSTAPD_MODULE_WPA,
...
static void wpa_group_setkeys(struct hostapd_data *hapd)
{
	...
	while (sta) {
		if (sta->wpa_sm) {
            if(sta->wpa_sm->sta_ptk_done==TRUE) {
                sta->wpa_sm->GUpdateStationKeys = TRUE;
                wpa_sm_step(sta->wpa_sm);
            }
            else {
                sta->wpa_sm->GUpdateStationKeys = FALSE;
                sm->GKeyDoneStations--;
            }
		}
		sta = sta->next;
