...
#ifdef BUILD_ATT_IEEE8021XAUTH
SMstat WPAState; 
int iteration=0; 
int state=0;
extern pid_t pid;
void eapol_sm_wpafunc(int state)
{
	FILE *FP=NULL; 
    FP = fopen(WPA_STATUSFILE_PATH,"w"); 
	if(FP!=NULL)
	{
		fprintf(FP,"%d.%d", state,iteration); 
		fclose(FP);
		if (pid > 0) 
		{ 
			kill(pid, SIGWINCH);
...
/*Netgear added change start*/
#ifdef BUILD_ATT_IEEE8021XAUTH
    iteration = iteration+1; /*For every restart and retry INITIALIZE will be the must state*/
	WPAState = WPAInitialize;
    eapol_sm_wpafunc(WPAState);
#endif
/*Netgear added change end*/
