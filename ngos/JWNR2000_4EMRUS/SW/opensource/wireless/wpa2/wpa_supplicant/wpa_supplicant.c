...
//Westell Change - send a msg to the dhcp client to renew the dhcp lease for ptm0
		if(strcmp(wpa_s->ifname, "ptm0")==0)
		{
		     char *line=NULL; 
      		     int len=0;
		     char cmd[128];
		     FILE *po;
		     unsigned int pid=0;
		     sprintf(cmd, "ps w | grep udhcpc");
		     po = popen(cmd, "r");
		     if(po)
		     {
		       while(getline(&line, &len, po) != EOF)
		       {
			   if(strstr(line, "ptm0"))
			   {
			     ...
			     const u8 *buf, size_t len)
{
    ...
    printf("%s: new: get mac address again, for ether-dongle mode\n", __func__);
