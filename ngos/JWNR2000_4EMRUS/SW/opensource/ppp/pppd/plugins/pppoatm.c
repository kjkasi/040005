...
#if 1 /* Westell Start */
	{
		info("PPPoATM setdevname_pppoatm(%s, , %d) device was already set",
				cp, doit);
		/* Allow this to be called into twice. */
		return 1;
	}
#else
		return 0;
...
#if 0 /* Westell Start - dont need this */
...
{
/* Westell Start */
#ifndef CONFIG_WSTL_AMAZONSE
	/* To close the connection when disconnect is called. */
	if(pppoa_fd > 0) {
		struct atm_backend_ppp be;
		be.backend_num = ATM_BACKEND_PPP_BCM_CLOSE_DEV;
		if (ioctl(pppoa_fd, ATM_SETBACKEND, &be) < 0)
			fatal("ioctl(ATM_SETBACKEND): cannot disconnect %m");		
	}
#endif
/* Westell End */
	close(pppoa_fd);
