...
#endif
	  printf("  PID  PPID  PRI  NI  Uid     VmSize Stat Command\n"); /* Westell added PPID and NI */
		...
		if (use_selinux)
		  {
			...
			len = printf("%5d %4d %-32s %s ", p->pid, p->pri, sbuf, p->state);
		}
		else
#endif
		if(p->rss == 0)
			len = printf("%5d %5d %4d %3d %-8.8s        %s ", p->pid, p->ppid, p->pri, p->nice, p->user, p->state); /* Westell added nice */
		else
			len = printf("%5d %5d %4d %3d %-8.8s %6ld %s ", p->pid, p->ppid, p->pri, p->nice, p->user, p->rss, p->state); /* Westell added nice */
