...
#ifdef FEATURE_EMAIL
#define SIG_SEND_FILE "/tmp/sigsend"
#define WEBSRV_PID_FILE "/var/run/webserver.pid"
#endif
/*************************NETGEAR END*********************************/
...
static char syslog_msg[MAXLINE + 1];
static int  counter_remote =1;
...
static void ipcsyslog_cleanup(void)
{
	...
	if (shmid != -1) {
		/* Westell Modifications Start */
		if((pFile=fopen("/tmp/SyslogLimits","r"))==NULL)
		{	
		shmctl(shmid, IPC_RMID, NULL);
		}
		else /* skip the shared memory segment removal */
		{
		fclose(pFile);
		}
		/* Westell Modifications End */
	}
	if (s_semid != -1) {
...
static void ipcsyslog_init(void)
{
	FILE *pFile=NULL; /* Westell Added */
	if (buf == NULL) {
		if ((shmid = shmget(KEY_ID, shm_size, IPC_CREAT | 511)) == -1) {
	...
	/* Westell Modifications Start */		
		...
		else /* dont reset the syslog buffer head and tail */
		{
			fclose(pFile);
			unlink("/tmp/SyslogLimits");
		}
	/* Westell Modifications End */
 ...
 *         checkLogBuffSize  
 * Description - 
 *         Function to get the verify the LOG buffer full condition
 *         and signals to send an email. 
 * Input -
 *			void
 * Return -
 *           void
...
static void checkLogBuffSize()
{
	FILE         *fp=NULL;
	char         line[256];
	unsigned int pid=0;
	int          tmp_size=0,desired_size=0, used_buff=0;

    memset(line,'\0',sizeof(line));
	sem_down(s_semid); /* lock the critical block */
	if(buf->size) { /* actuall shared memory size */
		tmp_size = (buf->size)/1024; /* size in KB*/
		desired_size = tmp_size*(1024-102); /*desired_size is 90 %= full-10%.*/
		if(buf->tail >= buf->head) { /* if the circular buffer has not rotated*/
			used_buff = (buf->tail)-(buf->head); 
		} else {
			used_buff = desired_size; /* the buffer is full and the circular buffer is rotated*/
		}
		sem_up(s_semid);	/*unlock the critical block*/
        if(used_buff >= desired_size) {
            if(access(SIG_SEND_FILE,R_OK)!=0){ /* verify if this file is already created*/

                fp = fopen(WEBSRV_PID_FILE,"r");

                if(fp == NULL) { 
                    printf(stderr,"%s:Unable to open websrv_pid file",__FUNCTION__);
                    return;
                } else {
                    while (fgets(line, sizeof(line), fp) && !feof(fp))
                    {
                        pid = strtoul(line, NULL, 10);
                        system("touch /tmp/sigsend"); /* create the temporary file before sending the signal */
                ...
                printf(stderr,"no signal sent");
            }
        } else {
            if(access(SIG_SEND_FILE,R_OK)==0){
                unlink(SIG_SEND_FILE);
            }
        }
	} else {
			printf(stderr,"shmem is null");
	}
}
...
/*Westell Modification Starts*/
#if (defined(BUILD_WSTL_ATT_VERSALINK) || defined(INCLUDE_ATT_GUI))
/*functionality to display time since reboot*/

#define TIME_LINE_SIZE	128
#define MAX_PROCESS_SIZE 16
static int 
GetTimeSinceBoot(char* timestamp)
{
  FILE *fp;
  char line[TIME_LINE_SIZE];
  char *lp;
  unsigned long days;
  unsigned long hours;
  ...
  if (fgets(line, TIME_LINE_SIZE, fp) == NULL)
  {
    fclose(fp);
    return FALSE;
  }
  ...
  sprintf(line, "+%#03d days %0.2d:%0.2d:%0.2d     ", days, hours, minutes, seconds);
...
char tmp_msg2[MAXLINE+1];   // The final log message to be displayed on the GUI
char tmp_msg3[MAXLINE+1];
char *padding = NULL;       //holds the padding string for each process
int pad_len = 0,a = 0;
char severity[6]="";        //holds the severity value ALR,NTC,ERR,WRN,INF,NTC etc
char *process_string = NULL;//holds the process name
char *junk = NULL;          //used to hold token values and retokenize.
char process[20] = "";      //holds the final process string
int process_len=0;          //length of the process 
#endif
#ifdef CONFIG_FEATURE_REMOTE_LOG
...
#ifdef INCLUDE_ATT_GUI
			if(strcmp(c_pri->c_name,"warn")==0)
			{
			    strcpy(severity,"WRN |");
			}
			if(strcmp(c_pri->c_name,"notice")==0)
			{
			    strcpy(severity,"NTC |");
			}
			if(strcmp(c_pri->c_name,"info")==0)
			{
			    strcpy(severity,"INF |");
			}
			if(strcmp(c_pri->c_name,"err")==0)
			{
			    strcpy(severity,"ERR |");
			}
			if(strcmp(c_pri->c_name,"alert")==0)
			{
			    strcpy(severity,"ALR |");
			}
			if(strcmp(c_pri->c_name,"debug")==0)
			{
			    strcpy(severity,"DBG |");
			}
			    snprintf(res, sizeof(res), "%s",severity);
#else			
	...
	if (strlen(msg) < 16 || msg[3] != ' ' || msg[6] != ' ' ||
		msg[9] != ':' || msg[12] != ':' || msg[15] != ' ') {
/*
**	#define SYSLOG_TO_USE_GM_TIME 1
*/
#ifdef SYSLOG_TO_USE_GM_TIME /* AT&T may want gmtime in the syslog */
		timeinfo = gmtime(&now);
#else
		timeinfo = localtime(&now);
#endif
	} else {
#ifdef SYSLOG_TO_USE_GM_TIME /* AT&T may want gmtime inthe syslog, this hack prevents fixing uClibc */
		timeinfo = gmtime(&now);
#else
		timeinfo = localtime(&now);
#endif
    ...
    strcpy(tmp_msg,msg);
    strcpy(tmp_msg3,msg);
    process_string=strtok(tmp_msg,":");
    if(process_string) 
    {
    	junk=strtok(process_string,"[");
    	...
    	{
        	strcpy(process,process_string);
    	}
    	process_len=strlen(process);
    }
	if(MAX_PROCESS_SIZE > process_len)
	a=(MAX_PROCESS_SIZE-process_len);
    padding=(char*)malloc(a+1);
	if(padding)
	{
    	for(pad_len=0;pad_len<a;pad_len++)
    	{
 	    	padding[pad_len]=' ';
    	}
		if(MAX_PROCESS_SIZE > process_len)
		{
    	padding[MAX_PROCESS_SIZE-process_len]='\0';
		}
		else if(process_len >= MAX_PROCESS_SIZE)
		{
    	padding[0]='\0';
		}
    	strcat(tmp_msg2,process);
    	strcat(tmp_msg2,padding);
    	strcat(tmp_msg2,"|");
    	junk=strtok(tmp_msg3,":");
    	if(junk)
		{
    		junk=junk+strlen(junk)+1;
    		strcat(tmp_msg2,junk);
		}
    	free(padding);
	}	
#endif
strftime(timestamp,24,"%Y/%m/%d %H:%M:%S %Z",timeinfo);
/*If timestamp contains the epoch year '2000' then use time since boot*/
	if(strstr(timestamp,"2000")!=NULL)
	{
		GetTimeSinceBoot(timestamp);
	}	
	/* todo: supress duplicates */
#ifdef INCLUDE_ATT_GUI
	if(strcmp(syslog_msg,msg)==0)
	{
		if(!first_log)
		{
...
#ifdef CONFIG_FEATURE_REMOTE_LOG
#ifdef INCLUDE_ATT_GUI
	if(counter_remote==1  && strcmp(syslog_msg,msg)!=0)
	{
		snprintf(line, sizeof(line), "<%d> %s", pri, msg);
   	}
	else if(counter_remote!=1 && strcmp(syslog_msg,msg)!=0)
	{
		snprintf(line,sizeof(line),"<%d> Previous message repeated %d times",pri,counter_remote);
	}   
...
#ifdef INCLUDE_ATT_GUI
	{
		...
		if(strcmp(syslog_msg,msg) != 0)
		{
			if(counter_local != 1)
			{
				message("%s %s %s %d %s \n", timestamp, "INF | syslogd         |","Previous message repeated",counter_local,"times");
				counter_local=1;
			}
		if (small)
			message("%s %s\n", timestamp, tmp_msg2);
		else
			message("%s %s %s\n", timestamp,res, tmp_msg2);
	   }
	}
	first_log=0;
	strcpy(syslog_msg,msg);
#else
	{
		/* now spew out the message to wherever it is supposed to go */
		if (small)
...
static void logMessage(int pri, char *msg)
{
	...
	#ifdef INCLUDE_NG_GUI
	char time_stamp[64];
	struct tm * timeinfo;//For Log change
	...
	if (strlen(msg) < 16 || msg[3] != ' ' || msg[6] != ' ' ||
		...
		/* NETGEAR Changes start */
		#ifndef INCLUDE_NG_GUI	
		timestamp = msg;
		timestamp[15] = '\0';
		#else
     	time(&now);
		timeinfo = localtime(&now);	
		strftime(time_stamp, 40,"%A, %B %d, %Y %H:%M:%S %Z",timeinfo);
...
/***********************************************NETGEAR START***********************************************/            
#ifdef FEATURE_EMAIL
                         checkLogBuffSize(); /* check the buffer size and send an e-mail when full upto 90%*/
#endif
/***********************************************NETGEAR END*************************************************/
			if(( -1 == sendto(remotefd, line, strlen(line), 0,
							(struct sockaddr *) &remoteaddr,
...
static void doSyslogd(void)
{
	...
	message("%s %s %s%s\n", time_stamp,"user.notice syslog: NGGUI:[Initialized, firmware version:",version,"]");		
	#endif
	/* NETGEAR Changes end */
...
int syslogd_main(int argc, char **argv)
{
	...
	char *p;
	/* NETGEAR Changes start :Added parameter 'v' */
	/* do normal option parsing */
	while ((opt = getopt(argc, argv, "m:nO:s:Sb:R:LC:v:")) > 0) {
	/* NETGEAR Changes end */
		switch (opt) {
		case 'm':
...
/* NETGEAR Changes end */	
	...
	if((access("/tmp/sigsend", R_OK)== 0)) {
		unlink("/tmp/sigsend");
	}
	if((access("/tmp/email_sent", R_OK)== 0)) {
        	unlink("/tmp/email_sent");
        }
