...
static long ntransmitted, nreceived, nrepeats, pingcount=5;
...
static void sendping(int junk)
{
	struct icmp *pkt;
	int i;
	char packet[datalen + 8]; //CR9622 - Force ICMP Header to 8 bytes instead of sizeof(struct icmp)];
...
static void ping(const char *host)
{
	...
	if (tos != 0)
    	{
    		/* set type of service if not default value */
    		sockopt = tos;
  		setsockopt (pingsock, IPPROTO_IP, IP_TOS, (char*)&sockopt, sizeof(sockopt));
  	}
