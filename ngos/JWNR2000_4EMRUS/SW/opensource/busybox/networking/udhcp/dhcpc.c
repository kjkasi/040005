...
#endif
{
	...
	int sig;
	int no_clientid = 0;
	char *colon=NULL, *p=NULL; //Westell Change
	uint8_t opt_type=0; 	   //Westell Change
	int *en_p, *iaid_p;        //Westell Change 
	...
	static const struct option arg_options[] = {
		...
		{"options",	required_argument,	0, 'x'}, //Westell Change		
		{0, 0, 0, 0}
	};
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "c:CV:fbH:h:F:i:np:qr:s:T:t:vx:", arg_options, &option_index);
		if (c == -1) break;

		switch (c) {
		case 'c':
//Westell Change - DUID based on enterprise number
			if (no_clientid) show_usage();
			len = strlen(optarg) > 255 ? 255 : strlen(optarg);
			free(client_config.clientid);
			client_config.clientid = xmalloc(len + 2 + 11);
			client_config.clientid[OPT_CODE] = DHCP_CLIENT_ID;
			client_config.clientid[OPT_LEN] = len+11;
			client_config.clientid[OPT_DATA] = 255;
			iaid_p = (int *)&client_config.clientid[OPT_DATA+1];
			*iaid_p = 1;
			client_config.clientid[OPT_DATA+5] = 0;
			client_config.clientid[OPT_DATA+6] = 2;
	        	en_p = (int *)&client_config.clientid[OPT_DATA+7];
	        	*en_p = ENTERPRISE_NUM;
			strncpy((char*)client_config.clientid + OPT_DATA + 11, optarg, len);
			break;
//Westell Change - to handle option 125
                case 'x':
		        colon = strchr(optarg, ':'); 
			p = optarg;
			while( colon && p != colon )
			{
			   if( *p>='0' && *p <= '9')
			      opt_type = opt_type*10 + (*p-'0'); 
			   p++;
			}
			if(opt_type == DHCP_VENDOR_IDENTIFYING)
			{

			   p++;
                           len = strlen(p) > 255 ? 255 : strlen(p);
			   if(client_config.options)
                             free(client_config.options);
                           client_config.options = xmalloc(len + 2 + 4 +1);
                           client_config.options[OPT_CODE] = opt_type;
                           client_config.options[OPT_LEN] = len + 4 +1;
			   en_p = (int *)&client_config.options[OPT_DATA];
			   *en_p = ENTERPRISE_NUM;
			   client_config.options[OPT_DATA+4] = len;
                           client_config.options[OPT_DATA+4+1] = '\0';
                           strncpy((char*)client_config.options+ OPT_DATA+4+1, p, len);
			}
                        break;
		case 'C':
...
//Netgear Change Start - to handle Telstra Requirement
#ifdef INCLUDE_EVG2000_GUI
		client_config.vendorclass = xmalloc(sizeof("Netgear DEVG2020") + 2);
		client_config.vendorclass[OPT_CODE] = DHCP_VENDOR;
		client_config.vendorclass[OPT_LEN] = sizeof("Netgear DEVG2020") - 1;
		client_config.vendorclass[OPT_DATA] = 1;
		memcpy(&client_config.vendorclass[OPT_DATA],
			"Netgear DEVG2020", sizeof("Netgear DEVG2020") - 1);
//Netgear Change End - to handle Telstra Requirement
//Netgear MVBR1000 Change Start - adding the MVBR specific vendor class id.
#elif INCLUDE_MVBR1000_SPECIFIC
		client_config.vendorclass = xmalloc(sizeof("Netgear MVBR1000") + 2);
		client_config.vendorclass[OPT_CODE] = DHCP_VENDOR;
		client_config.vendorclass[OPT_LEN] = sizeof("Netgear MVBR1000") - 1;
		client_config.vendorclass[OPT_DATA] = 1;
		memcpy(&client_config.vendorclass[OPT_DATA],
			"Netgear MVBR1000", sizeof("Netgear MVBR1000") - 1);
//Netgear MVBR1000 Change End
#else
