...
//Westell Change - 6rd support
static void fill_6rdprefix(uint8_t *sixrdprefix, uint8_t *raw_sixrdprefix, uint8_t len)
{
	int i;
	uint8_t num_of_byte = len/8, last_mask=0xff;

	memset(sixrdprefix, 0, 16);

	for(i=0; i<num_of_byte; i++)
	{
	   sixrdprefix[i] = raw_sixrdprefix[i];
	}

	last_mask <<= (8-len%8);
	sixrdprefix[i] = raw_sixrdprefix[i] & last_mask;
...
static char **fill_envp(struct dhcpMessage *packet)
{
	...
	envp = xcalloc(sizeof(char *), num_options + 9); //Westell Change - 6rd support
	j = 0;
	...
	for (i = 0; dhcp_options[i].code; i++) {
	     if (!(temp = get_option(packet, dhcp_options[i].code)))
			continue;
//Westell Change - 6rd support
	     if (dhcp_options[i].code == DHCP_OPTION_6RD) {
		  uint8_t ipv4_mask_len = temp[0], 
			  sixrd_prefix_len=temp[1],
			  sixrd_prefix[16];
		  uint16_t val_u16;
		  uint8_t *dest, *temp_ip, k, prefix_size;

		  envp[j] = xmalloc(strlen("ipv4_mask_len=")+4);
	          asprintf(&envp[j++], "ipv4_mask_len=%u", ipv4_mask_len);

		  envp[j] = xmalloc(strlen("prefix_len=")+4);
	          asprintf(&envp[j++], "prefix_len=%u", sixrd_prefix_len);

		  fill_6rdprefix(sixrd_prefix, &temp[2], sixrd_prefix_len);
	
		  envp[j] = xmalloc(strlen("sixrd_prefix=")+40);
		  dest = envp[j++];
		  dest += sprintf(dest, "sixrd_prefix=");
		  prefix_size = sixrd_prefix_len/16;
		  if( sixrd_prefix_len%16) prefix_size++;
		  for(k=0; k<prefix_size; k++)
		  {
		      if(k>0) dest += sprintf(dest, ":");
		      memcpy(&val_u16, &sixrd_prefix[2*k], 2);
		      dest += sprintf(dest, "%04x", val_u16);
		  }
		  
		  envp[j] = xmalloc(sizeof("br_ipv4_addr=")+sizeof("255.255.255.255 "));
		  dest = envp[j++];
                  dest += sprintf(dest, "br_ipv4_addr=");
		  temp_ip = &temp[18];
		  dest += sprintf(dest, "%d.%d.%d.%d ", temp_ip[0], temp_ip[1], temp_ip[2], temp_ip[3]);
