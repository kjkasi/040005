      ...
      unsigned int *flags, struct ebt_entry_match **match)
{
	...
	case VLAN_ID:
		...
		local.id = (unsigned short)strtoul(argv[optind - 1], &end, 10);
	...
	case VLAN_PRIO:
		...
		CHECK_IF_MISSING_VALUE;
		local.prio = (unsigned char)strtoul(argv[optind - 1], &end, 10);
	...
	case VLAN_ENCAP:
		...
		CHECK_IF_MISSING_VALUE;
		 local.encap = (unsigned short)strtoul(argv[optind - 1], &end, 16);
		if (*end != '\0') {
