		    ...
		    int rewrite, int sync)
{
	...
	/* This should include the current lease. */
        /* Netgear: Max lease after which lease file is re-written is modified to 5 from 20. */
        /* This is done to keep the size in /var/etc of dhclient.lease file under check */
	if (!rewrite && (leases_written++ > 5)) {
		rewrite_client_leases();
		leases_written = 0;
		return ISC_R_SUCCESS;
	}
