...
/* Westell Changes */
#if !defined(DHCPV6_SUPPORT_IA_NA)
/* ATT reqm no. 656 --> The device MUST NOT support other DHCPv6 server
       functionality other than that required for prefix delegation. For
       example, it should not respond to a Solicit message for IP address
       assignment nor to an Information-request message */

	oc = lookup_option(&dhcpv6_universe, packet->options, D6O_IA_NA);
	if (oc != NULL) {
            log_debug("Discarding Address Request (option IA_NA) in Solicit;");
            goto exit;
        }
#endif
/* Westell Changes End */
...
/* ATT reqm no. 656 --> The device MUST NOT support other DHCPv6 server
       ...
       example, it should not respond to a Solicit message for IP address
       assignment nor to an Information-request message */
#if defined(DHCPV6_SUPPORT_INFORMATION_REQUEST)
