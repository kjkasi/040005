...
/* Netgear Changes start */
	if(!strcmp(p_self->info.dyndns_server_name.name, MEM_3322_SERVER_NAME)) { /* It is 3322.org update request */

		return sprintf(p_self->p_req_buffer, ORG_3322_GET_MY_IP_HTTP_REQUEST_FORMAT,
	...
	} else { /* It is dyndns.org update request */
/* Netgear Changes start */
...
/* Westell added get/set persistent_ip */
/* get_last_persistent_ip
	executes the external command "persist_cmd"
	the last IP and updated time should be returned on each on there own line.
	the time is in seconds since the EPOCH.
*/
static RC_TYPE get_last_persistent_ip(DYN_DNS_CLIENT *p_self)
{
	FILE *cmd;
	time_t last_update_tm, last_rec_update, curr_time;
	char *p, buff[32], new_ip_str[IP_V4_MAX_LENGTH+1];

	buff[0] = new_ip_str[0] = '\0';

	if (p_self->persist_cmd == NULL)
		return RC_OK;
	if ((cmd = popen(p_self->persist_cmd, "r")) == NULL)
		return RC_OK;
	fgets(new_ip_str, IP_V4_MAX_LENGTH, cmd);
	fgets(buff, 31, cmd);
	pclose(cmd);
	if (new_ip_str == '\0')
		return RC_OK;
	...
	last_rec_update = (time_t) strtol(buff, &p, 10);
	last_update_tm = curr_time - last_rec_update;

	if (last_update_tm > p_self->forced_update_period_sec)
	{
		/* persistant time stamp indicates we missed a forced update */
		p_self->times_since_last_update = p_self->forced_update_times + 1;
	}
	else
	{
		/* re-construct times_since_last_update as if we were running */
		p_self->times_since_last_update = last_update_tm / p_self->sleep_sec;
	}

	DBG_PRINTF((LOG_WARNING,"I:" MODULE_TAG "IP (%s) last updated (%.24s) times=%d/%d intvl=%d\n", new_ip_str, ctime(&last_rec_update), p_self->times_since_last_update, p_self->forced_update_times, p_self->sleep_sec));

	return RC_OK;
}

/* set_persistent_ip
	executes the external command "persist_cmd IP time"
	where IP is the last updated IP
	and time ins the time of last update in seconds since EPOCH
*/
/* Netgear Chnages Starts Here */
//static RC_TYPE set_persistent_ip(DYN_DNS_CLIENT *p_self)
static RC_TYPE set_persistent_ip(DYN_DNS_CLIENT *p_self,char *status)
...
/* Netgear Chnages Starts Here */
    /* If the IP address is not reachable, generally the value will be null,
    in such cases make ip address to 0.0.0.0 avoid problems at script */
    if(p_self->info.my_ip_address.name[0] == '\0')
    {
        strcpy(p_self->info.my_ip_address.name, "0.0.0.0");
    }
//	ncmd = malloc(strlen(p_self->persist_cmd) + strlen(p_self->info.my_ip_address.name) + 3 + 11);
//	sprintf(ncmd, "%s %s %ld", p_self->persist_cmd, p_self->info.my_ip_address.name, update_tm);
	ncmd = malloc(strlen(p_self->persist_cmd) + strlen(p_self->info.my_ip_address.name) + 3 + 11 + STATUS_MSG_LENGTH);
	sprintf(ncmd, "%s %s %ld %s", p_self->persist_cmd, p_self->info.my_ip_address.name, update_tm,status);
/* Netgear Chnages Ends Here */
	rc = system(ncmd);
	free(ncmd);
	DBG_PRINTF((LOG_WARNING,"I:" MODULE_TAG "RC from persist %d, IP %s time %ld\n", rc, p_self->info.my_ip_address.name, update_tm));
	return RC_OK;
}
...
{
	int i, update_persistent;
	RC_TYPE rc = RC_OK;
	/* Netgear Chnages Starts Here */
	char status[STATUS_MSG_LENGTH] = {0};
	...
	{			
		for (i = 0; i < p_self->alias_info.count; ++i)
		{
			...
			/*build dyndns transaction*/
			{
				...
				if (rc == RC_OK)
				{
					BOOL update_ok = p_self->info.p_dns_system->p_rsp_ok_func((struct _DYN_DNS_CLIENT*)p_self,
					...
					if (update_ok)
					{
						...
						/* Netgear Chnages Starts Here */
							...
							set_persistent_ip(p_self,status);
//							set_persistent_ip(p_self);
						/* Netgear Chnages Ends Here */
						}
					...
					else
					{
						...
						{
							p_self->sleep_sec = 3600;
							failCount = 0;
						}
						
						http_tr.p_rsp[http_tr.rsp_len] = 0;
						if(strstr(http_tr.p_rsp,"badauth") || 
							strstr(http_tr.p_rsp, "UNAUTHORIZED"))
						{
							strcpy(status,"\"badauth");
						}
						else if(strstr(http_tr.p_rsp,"nohost"))
						{
							strcpy(status,"\"nohost");
						}
						else
						...
						time_t update_tm;
						update_tm = time((time_t *) 0);
						sprintf(status,"%s at %ld\"",status, update_tm);
...
/* Netgear Chnages Starts Here */
#ifdef INCLUDE_NG_GUI
            /* If the IP address is reachable (in such cases like DNS is wrong or not reachable),
               we will have to make the status as Failed, and pass the same to script for cfg_write */
            strcpy(status,"\"Failed");
