...
static RC_TYPE get_set_persistent_ip_cmd(CMD_DATA *p_cmd, int current_nr, void *p_context);
...
static CMD_DESCRIPTION_TYPE cmd_options_table[] = 
{
	...
	{"--persist", 1, {get_set_persistent_ip_cmd, NULL}, "\"cmd [IP]\" - external command to get or set the last IP"},
...
static RC_TYPE get_set_persistent_ip_cmd(CMD_DATA *p_cmd, int current_nr, void *p_context)
{
	...
	p_self->persist_cmd = malloc(strlen(p_cmd->argv[current_nr])+1);
	strcpy(p_self->persist_cmd, p_cmd->argv[current_nr]);
