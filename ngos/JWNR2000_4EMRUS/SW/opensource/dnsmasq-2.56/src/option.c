...
blkSites_t *blkSitesList;			/* This list maintains the block site/keyword blocking strings */
blkTrustedIp_t *blkTrustedIpList;		/* This list maintains the trusted ip's */
#endif
#ifdef FEATURE_TRIPLEPLAY
lanDhcpOptionList_t *lanDhcpOptionList;
#endif
/* NETGEAR Changes End*/
...
#define LOPT_PARENTALCONTROL 302
#endif
/* NETGEAR Changes End*/
#define LOPT_MAXPORT   303

/* NETGEAR Changes Start*/
#if defined(FEATURE_BLKSITES)
#define LOPT_BLKSITES 304
#ifdef FEATURE_BLKSITESBANNER
#define LOPT_BANNERSRVIP 305
#endif
#define LOPT_BLKSITESTRUSTEDIP 306
#endif
#if defined(FEATURE_AUTOBRIDGELAN2WAN)
#define LOPT_IGNRDHCP 307
#endif
#ifdef FEATURE_TRIPLEPLAY
#define LOPT_LANDHCPOPTION60 308 
#endif
/* NETGEAR Changes End*/
...
/* NETGEAR Changes Start*/
#if defined(FEATURE_PARENTALCONTROL)
    { "parentalcontrol", 1, 0, LOPT_PARENTALCONTROL },
#endif
#if defined(FEATURE_BLKSITES)
    { "blksites", 1, 0, LOPT_BLKSITES },
#ifdef FEATURE_BLKSITESBANNER
    { "bannersrvip", 1, 0, LOPT_BANNERSRVIP },
#endif
    { "trustedip", 1, 0, LOPT_BLKSITESTRUSTEDIP},
#endif
#if defined(FEATURE_AUTOBRIDGELAN2WAN)
    { "ignoreDHCPWithOptionValue", 1, 0, LOPT_IGNRDHCP },
#endif
#ifdef FEATURE_TRIPLEPLAY
    { "LanDHCPOption60", 1, 0, LOPT_LANDHCPOPTION60 },
#endif
/* NETGEAR Changes End*/
...
/* NETGEAR Changes Start*/
#if defined(FEATURE_PARENTALCONTROL)
  { LOPT_PARENTALCONTROL, ARG_ONE, "number", gettext_noop("Specify whether Parental control Enabled or Disabled by 0 or 1."), NULL },
#endif
#if defined(FEATURE_BLKSITES)
  { LOPT_BLKSITES, ARG_ONE, "blocksites", gettext_noop("Give the list of blocksites seperated by commas."), NULL },
#ifdef FEATURE_BLKSITESBANNER
  { LOPT_BANNERSRVIP, ARG_ONE, "bannersrvip", gettext_noop("Give the ip address of the tcp server which handles block site http requests."), NULL },
#endif
  { LOPT_BLKSITESTRUSTEDIP, ARG_ONE, "trustedip", gettext_noop("Give the list of trusted ip details which can pass through blocksites."), NULL },
#endif
#if defined(FEATURE_AUTOBRIDGELAN2WAN)
  { LOPT_IGNRDHCP, ARG_ONE, "optionwithstring", gettext_noop("Specify DHCP option and string to drop the packet."), NULL },
#endif
#ifdef FEATURE_TRIPLEPLAY
  { LOPT_LANDHCPOPTION60, ARG_ONE, "landhcpoption60", gettext_noop("Specify DHCP option 60 vendor class Ids of the LAN device."), NULL },
#endif
/* NETGEAR Changes End*/
  { 0, 0, NULL, NULL, NULL }
}; 
...
/* NETGEAR Changes Start*/
#if defined(FEATURE_PARENTALCONTROL)
    case LOPT_PARENTALCONTROL:  /* parental control enable or not */
      if (!atoi_check(arg, &enable))
        {
                syslog(LOG_ERR,"%s:%d:setting add-mac enable \n",__FUNCTION__,__LINE__);
                option = '?';
        }
      else
        {
		daemon->parentalcontrol = enable;
/* Currently blocking this code as we see it creating issue */
/*
		if(daemon->parentalcontrol)
		{
			syslog(LOG_ERR,"%s:%d:setting add-mac enable \n",__FUNCTION__,__LINE__);
		...
		if(!createDeviceList())
		{
			syslog(LOG_ERR,"%s:%d:Error in creating the list \n",__FUNCTION__,__LINE__);
		}
*/
        }
      break;
...
#if defined(FEATURE_BLKSITES)
    case LOPT_BLKSITES:  
	daemon->blksites = opt_string_alloc(arg);

	char *result = NULL;
	blkSitesList = NULL;

	result = strtok( daemon->blksites , ",");

	while( result != NULL )
	{
		blkSites_t *newnode = NULL;

		if(!(newnode=malloc(sizeof(blkSites_t))))	
		{
			syslog(LOG_ERR,"Unable to assign memory.");
			break;
		}
		
		memset(newnode->keyword,0,sizeof(newnode->keyword));
		strcpy(newnode->keyword,result);
		newnode->next = (blkSites_t *) blkSitesList;

		blkSitesList = newnode;

		result = strtok( NULL, "," );
	}

     break;
    case LOPT_BLKSITESTRUSTEDIP:  
	daemon->trustedip = opt_string_alloc(arg);

	char *trustedIp = NULL;
	blkTrustedIpList = NULL;

	trustedIp = strtok( daemon->trustedip , ",");

	while( trustedIp != NULL )
	{
		blkTrustedIp_t *newnode = NULL;

		if(!(newnode=malloc(sizeof(blkTrustedIp_t))))	
		{
			syslog(LOG_ERR,"Unable to assign memory.");
			break;
		}
		...
		strcpy(newnode->ip,trustedIp);
		newnode->next = (blkTrustedIp_t *) blkTrustedIpList;

		blkTrustedIpList = newnode;
		trustedIp = strtok( NULL, "," );
	}

     break;
#ifdef FEATURE_BLKSITESBANNER
    case LOPT_BANNERSRVIP:  
	daemon->bannersrvip = opt_string_alloc(arg);
	break;
#endif
#endif
#if defined(FEATURE_AUTOBRIDGELAN2WAN)
    case LOPT_IGNRDHCP:
	{
        char *ignoredhcp = opt_string_alloc(arg);
        daemon->ignrdhcp_option = strtok( ignoredhcp, ",");
        daemon->ignrdhcp_value = strtok( NULL, "," );
        break;
	}
#endif

#ifdef FEATURE_TRIPLEPLAY
    case LOPT_LANDHCPOPTION60:
    {
        daemon->landhcpoption60 = opt_string_alloc(arg);
        lanDhcpOptionList = NULL;
        result = strtok(daemon->landhcpoption60, "," );
        while( result != NULL )
        {
            lanDhcpOptionList_t *newnode = NULL;
            if(!(newnode=malloc(sizeof(lanDhcpOptionList_t))))
            {
                my_syslog(MS_DHCP | LOG_ERR,_("Unable to assign memory."));
                break;
            }
            memset(newnode->dhcpoptionvalue,0,sizeof(newnode->dhcpoptionvalue));
            strncpy(newnode->dhcpoptionvalue,result,strlen(result));
            newnode->next = (lanDhcpOptionList_t *) lanDhcpOptionList;
            lanDhcpOptionList = newnode;
            result = strtok( NULL, "," );
      	    my_syslog(MS_DHCP | LOG_INFO, _("Allowed DHCP option values for IPTV: %s "), newnode->dhcpoptionvalue);
        }
        break;
	}
#endif
