...
/* NETGEAR changes end*/
  ...
  if (leasestream)
/* NETGEAR Changes start */
 while (fgets(buff,HOST_LEASE_SIZE,leasestream) != NULL)
 {
        if(sscanf(buff,"%lu %255s %16s %255s %764s",&ei, daemon->dhcp_buff2, daemon->namebuff,daemon->dhcp_buff, daemon->packet) == 5)
        {
/* NETGEAR Changes end*/
...
/* NETGEAR Changes Start */
          ...
          ourprintf(&err, " ");
          my_syslog(LOG_INFO, _("Tripleplay: Writing Service Info"));
          if(lease->serviceID && (strlen(lease->serviceID)))
                ourprintf(&err, "%s", lease->serviceID);
          else
                ourprintf(&err, SERVICEID_BROADBAND);	  
#endif
/* NETGEAR Changes End */
	  ...
	  my_syslog(LOG_INFO, _("TR-111: writing device info"));
	  if(lease->opt125_vendor_oui && (strlen(lease->opt125_vendor_oui)))
	  	ourprintf(&err, "%s", lease->opt125_vendor_oui);
	  else
	  	ourprintf(&err, "*");
	  ourprintf(&err, " ");

	  if(lease->opt125_serial_num && (strlen(lease->opt125_serial_num)))
	  	ourprintf(&err, "%s", lease->opt125_serial_num);
	  else
	  	ourprintf(&err, "*");
	  ourprintf(&err, " ");

	  if(lease->opt125_product_class && (strlen(lease->opt125_product_class)))
	  	ourprintf(&err, "%s\n", lease->opt125_product_class);
	  else
	  	ourprintf(&err, "*\n");
	  my_syslog(LOG_INFO, _("TR-111: writing device info done"));
#endif
	}
...
/* NETGEAR Changes Start */
#ifdef FEATURE_TRIPLEPLAY
    my_syslog(MS_DHCP | LOG_INFO, _("lease_file_updated notification sent: %d"), __LINE__);
    system("cfg_msg dhcpleases_notify lease_file_updated");
#endif
/* NETGEAR Changes End */
...
/* NETGEAR Changes Start */
      ...
      if ((lease->expires != 0 && difftime(now, lease->expires) > 0) || lease == target || !icmp_ping(lease->addr))
#else
...
#define LEGAL_USERNAME_CHARS    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_"

/******************************************************************************
**
** FUNCTION:   MCFG_ConvertTo3986Percent
**
** DESCRIPTION:  function to convert illegal characters to escape sequences, per RFC
**
**
** PARAMETERS:
**
...
int MCFG_ConvertTo3986Percent(char *inputBuf, char *outputBuf, int len, char *legalCharSet)
{
    static char HexDigs[] = "0123456789ABCDEF";
    int out = 0, in = 0;
    char c;
    char defaultCharSet[] = LEGAL_USERNAME_CHARS;

    if (legalCharSet == NULL)
        legalCharSet = defaultCharSet;

    if ((inputBuf == NULL) || (outputBuf == NULL)) {
        return (0);
    }

    while ((c=inputBuf[in++]) != 0) {
        if (strchr(legalCharSet, c) != NULL) {
            /* The character is legal as is, add if there is room */
            if ((out+1) >= len) {
                return (0);
            }
            outputBuf[out++] = c;
        ...
        else
        {
            ...
            outputBuf[out++] = '%';
            outputBuf[out++] = HexDigs[(c>>4) & 0xF];
            outputBuf[out++] = HexDigs[c & 0xF];
        }
    }
    /* Add Null Terminator */
    outputBuf[out] = 0;
...
void lease_set_vendor_option(struct dhcp_lease *lease, char *OUI, char *SerialNumber, char *ProductClass)
{
  char *newOUI = NULL, *newSerialNumber = NULL, *newProductClass = NULL;
  
  if (!OUI && !SerialNumber && !ProductClass)
    return;
  

  if (OUI)
  {
      if (!newOUI && (newOUI = whine_malloc(strlen(OUI) + 1)))
	strcpy(newOUI, OUI);
  }

  if (SerialNumber)
  {
      if (!newSerialNumber && (newSerialNumber = whine_malloc(strlen(SerialNumber) + 1)))
	strcpy(newSerialNumber, SerialNumber);
  }

  if (ProductClass)
  {
      char ProductClass_temp[65];
      MCFG_ConvertTo3986Percent(ProductClass, ProductClass_temp, 65, NULL);

      if (!newProductClass && (newProductClass = whine_malloc(strlen(ProductClass_temp) + 1)))
        	strcpy(newProductClass, ProductClass_temp);
  }

  if (lease->opt125_vendor_oui)
    free(lease->opt125_vendor_oui);
  if (lease->opt125_serial_num)
    free(lease->opt125_serial_num);
  if (lease->opt125_product_class)
    free(lease->opt125_product_class);
  
  lease->opt125_vendor_oui = newOUI;
  lease->opt125_serial_num = newSerialNumber;
  lease->opt125_product_class = newProductClass;
