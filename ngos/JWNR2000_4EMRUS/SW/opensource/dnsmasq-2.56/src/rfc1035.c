...
/* NETGEAR Changes Start */
#if defined(FEATURE_PARENTALCONTROL)
	char data[64],macid[12];
        unsigned char lmac[12],resmac[6],resdev[8];
	/* first 11 bytes are for OpenDNS and remaining 8 are for the default device id 0000111111111111 */
	unsigned char devData[] = {0x00,0x04,0x00,0x0f,0x4f,0x70,0x65,0x6e,0x44,0x4e,0x53,0x00,0x00,0x11,0x11,0x11,0x11,0x11,0x11};
	char *devId = NULL;
	FILE *fp = NULL;
  ...
  if(fp!=NULL)
  {
	  ...
	  while((fgets(data,sizeof(data),fp)) != NULL)
	  {
		  ...
		  *devId = '\0';
		  ...
		  hex_decode(macid,strlen(macid),resmac);

		  if(lmac[0] == resmac[0] && lmac[1] == resmac[1] && lmac[2] == resmac[2] && lmac[3] == resmac[3] && lmac[4] == resmac[4] 
				  && lmac[5] == resmac[5]) 
		  {
		  	  /*syslog(LOG_DEBUG,"devId:%s:%d",devId,__LINE__);*/
			  hex_decode(devId,strlen(devId),resdev);
			  /* Prepended OpenDNS along with Device ID in devData*/
			  devData[0] =  0x00;
			  devData[11] =  resdev[0];
			  devData[12] =  resdev[1];
			  devData[13] =  resdev[2];
			  ...
			  devData[17] =  resdev[6];
			  devData[18] =  resdev[7];
			  /*syslog(LOG_DEBUG,"%d:Got the device id for this mac:devid: %x %x %x %x %x %x %x %x",__LINE__,resdev[0],resdev[1],resdev[2],resdev[3],resdev[4],resdev[5],resdev[6],resdev[7]);*/
			  break;
		  }
		  memset(data,0,64);
	  }
