...
#ifdef INCLUDE_TR_111
#define OPTION_VENDOR_SPECIFIC_DATA	125
#endif
...
#ifdef INCLUDE_TR_111
#define ENTERPRISE_NUMBER 3561

static char gatewayOUI[] = "00600F";
char gatewaySerialNumber[256], gatewayProductClass[256];
#endif
/* NETGEAR Changes Start */
#ifdef FEATURE_TRIPLEPLAY
extern lanDhcpOptionList_t *lanDhcpOptionList;
#endif
/* NETGEAR Changes End */

#define BRDBAND_FORUM_IANA       3562 /* Broadband forum IANA enterprise */
...
#ifdef INCLUDE_TR_111
static unsigned int get_uint(unsigned char *opt, int size);
static void option_put_vendor(struct dhcp_packet *mess, unsigned char *end, int opt, int len, unsigned int val);
static void read_gateway_info(void);
#endif
		  ...
		  size_t sz, time_t now, int unicast_dest, int *is_inform, int pxe)
{
...
#ifdef INCLUDE_TR_111
  unsigned int option_125_Vendor_Info = 0;
  char deviceOUI[256], deviceSerialNumber[256], deviceProductClass[256];
  ...
  /* NETGEAR Changes start */
#ifdef FEATURE_TRIPLEPLAY
  unsigned int option_60_Vendor_Info = 0;
#endif	
  #ifdef INCLUDE_NG_GUI
  struct in_addr a;
  char maclen[32];
  void *addr;
  ...
  if ((opt = option_find(mess, sz, OPTION_VENDOR_ID, 1)))
  {
    ...
    option_60_Vendor_Info = 1;
  }
#endif
  /* NETGEAR Changes End */
#ifdef INCLUDE_TR_111
    my_syslog(LOG_INFO, _("TR-111: Before TR-111 handling of DHCP packet"));
    if ((opt = option_find(mess, sz, OPTION_VENDOR_SPECIFIC_DATA, 0)))
	{
		unsigned int iDone = 0, iSubDataLen = 0;
		unsigned int uiDataLen = 0;
		char *pzData = NULL;

		my_syslog(LOG_INFO, _("TR-111: Reading vendor specific data from the dhcp packet"));

		for(pzData = option_ptr(opt,0), uiDataLen = option_len(opt) -2; uiDataLen > (get_uint(pzData + 4, 1) + 5);
			uiDataLen-=(get_uint(pzData + 4, 1) + 5), pzData+=(get_uint(pzData + 4, 1) + 5))
				if(ENTERPRISE_NUMBER == get_uint(pzData, 4))
					break;

		if(ENTERPRISE_NUMBER == get_uint(pzData, 4))
		{
			my_syslog(LOG_INFO, _("TR-111: DSL forum enterprise number found"));
			uiDataLen = get_uint(pzData + 4, 1);
			pzData += 5;

			while(!iDone && pzData)
			{
				iSubDataLen = get_uint(pzData + 1, 1);
				if(1 == get_uint(pzData, 1))
				{
					memcpy(deviceOUI, pzData + 2, ((iSubDataLen < 255)?iSubDataLen:255) );
					deviceOUI[((iSubDataLen < 255)?iSubDataLen:255)] = '\0';
				}
				else if(2 == get_uint(pzData, 1))
				{
					memcpy(deviceSerialNumber, pzData + 2, ((iSubDataLen < 255)?iSubDataLen:255));
					deviceSerialNumber[((iSubDataLen < 255)?iSubDataLen:255)] = '\0';
				}
				else if(3 == get_uint(pzData, 1))
				{
					memcpy(deviceProductClass, pzData + 2, ((iSubDataLen < 255)?iSubDataLen:255));
					deviceProductClass[((iSubDataLen < 255)?iSubDataLen:255)] = '\0';
				}

				iSubDataLen = iSubDataLen + 2;
				if(uiDataLen > iSubDataLen){
					pzData+=iSubDataLen;
					uiDataLen -= iSubDataLen;
				}
				else{
					iDone = 1;
					pzData = NULL;
			...
			option_125_Vendor_Info = 1;
			my_syslog(LOG_INFO, _("TR-111: Got the vendor specific Device Info"));
		}
		my_syslog(LOG_INFO, _("TR-111: Done reading vendor specific Device Info"));
	}
	my_syslog(LOG_INFO, _("TR-111: After TR-111 handling of DHCP packet"));
#endif
  ...
  */
...
#if defined(FEATURE_AUTOBRIDGELAN2WAN)
    if(daemon->ignrdhcp_option != NULL && daemon->ignrdhcp_value != NULL)
    {
        if((opt = option_find(mess, sz, atoi(daemon->ignrdhcp_option), strlen(daemon->ignrdhcp_option))) 
            && strncmp(option_ptr(opt, 0), daemon->ignrdhcp_value, strlen(daemon->ignrdhcp_value)) == 0)
        {
            my_syslog(LOG_INFO, _("AUTOBRIDGELAN2WAN Option %s Value %s: Ignoring DHCP request"),daemon->ignrdhcp_option,daemon->ignrdhcp_value);
            return 0;
        }
    }
#endif
  ...
  switch (mess_type)
...
#ifdef INCLUDE_TR_111
      if (!lease && (lease = lease_find_by_addr(addr)) && option_125_Vendor_Info){
	  	my_syslog(LOG_INFO, _("TR-111: Lease found for address %s update the Device info"), inet_ntoa(addr));
	  	lease_set_vendor_option(lease, deviceOUI, deviceSerialNumber, deviceProductClass);
      	}
#endif
...
#ifdef INCLUDE_TR_111
	if(option_125_Vendor_Info)
	{
		int uiDataLen = strlen(gatewayOUI) + strlen(gatewaySerialNumber) + strlen(gatewayProductClass) + 11;
		my_syslog(LOG_INFO, _("TR-111: Add the gateway info in the offer packet"));
		option_put_vendor(mess, end, OPTION_VENDOR_SPECIFIC_DATA, uiDataLen, ENTERPRISE_NUMBER);
	}
	my_syslog(LOG_INFO, _("TR-111: Done with offer packet"));
#endif
      ...
      else
	{
	  ...
	  /* NETGEAR Changes Start */
#ifdef FEATURE_TRIPLEPLAY
	  if( option_60_Vendor_Info )
	  {
	    while(lanDhcpOptionList != 0)
	    {		
	      my_syslog(MS_DHCP | LOG_INFO, _("DHCP options value(%s)"), lanDhcpOptionList->dhcpoptionvalue);
	      my_syslog(MS_DHCP | LOG_INFO, _("DHCP options value from packet(%s)"),daemon->dhcp_buff3);
	      if(strncasecmp(daemon->dhcp_buff3, lanDhcpOptionList->dhcpoptionvalue, strlen( lanDhcpOptionList->dhcpoptionvalue)) == 0)
	      {
	        strncpy(lease->serviceID, SERVICEID_IPTV, strlen(SERVICEID_IPTV)); 
	        my_syslog(MS_DHCP | LOG_INFO, _("DHCP options value(%s) matches with dhcp packet(%s)"), lanDhcpOptionList->dhcpoptionvalue, daemon->dhcp_buff3);
	        break;
	      }
	      else
	      {
	        strncpy(lease->serviceID, SERVICEID_BROADBAND, strlen(SERVICEID_BROADBAND)); 
	        my_syslog(MS_DHCP | LOG_INFO, _("DHCP options value(%s) do not match with dhcp packet(%s)"), lanDhcpOptionList->dhcpoptionvalue, daemon->dhcp_buff3);
	      }
	      lanDhcpOptionList = lanDhcpOptionList->next;
	    }
	  }
#endif
	  /* NETGEAR Changes End */
#ifdef INCLUDE_TR_111
	  if(option_125_Vendor_Info){
	  	my_syslog(LOG_INFO, _("TR-111: Lease found update the Device info"));
	  	lease_set_vendor_option(lease, deviceOUI, deviceSerialNumber, deviceProductClass);
	  }
#endif
	  lease_set_interface(lease, int_index);
	  ...
	  print_mac(maclen, emac,emac_len);

	  my_syslog(MS_DHCP | LOG_INFO, "NGGUI:[DHCP IP:%s] to MAC address %s",inet_ntoa(a),maclen);
      #endif
	  /* NETGEAR Changes end */	
	  ...
	  {
		int uiDataLen = strlen(gatewayOUI) + strlen(gatewaySerialNumber) + strlen(gatewayProductClass) + 11;
		my_syslog(LOG_INFO, _("TR-111: Add the gateway info in the ACK packet"));
		option_put_vendor(mess, end, OPTION_VENDOR_SPECIFIC_DATA, uiDataLen, ENTERPRISE_NUMBER);
	  }
	  my_syslog(LOG_INFO, _("TR-111: Done with ACK packet"));
#endif
...
#ifdef INCLUDE_TR_111
      if (!lease && (lease = lease_find_by_addr(mess->ciaddr)) && option_125_Vendor_Info){
	  	my_syslog(LOG_INFO, _("TR-111: Lease found for address %s update the Device info"), inet_ntoa(mess->ciaddr));
	  	lease_set_vendor_option(lease, deviceOUI, deviceSerialNumber, deviceProductClass);
      	}
#endif	        
...
static void option_put_vendor(struct dhcp_packet *mess, unsigned char *end, int opt, int len, unsigned int val)
{
        ...
        if (p)
        {
                for (i = 0; i < 4; i++) /*enterprise number size is 4*/
                        *(p++) = val >> (8 * (4 - (i + 1)));

                /* Write OUI */
                *(p++) = (unsigned int)(len - 5);
                *(p++) = (unsigned int)(4);
                *(p++) = (unsigned int)(strlen(gatewayOUI));
                memcpy(p, gatewayOUI, strlen(gatewayOUI));
                p+=strlen(gatewayOUI);
                /* Write Gateway Serial Number */
                *(p++) = (unsigned int)(5);
                *(p++) = (unsigned int)(strlen(gatewaySerialNumber)-1);
                memcpy(p, gatewaySerialNumber, (strlen(gatewaySerialNumber)-1));
                p+=(strlen(gatewaySerialNumber)-1);
                /* Write Product Class */
                *(p++) = (unsigned int)(6);
                *(p++) = (unsigned int)(strlen(gatewayProductClass)-1);
                memcpy(p, gatewayProductClass, (strlen(gatewayProductClass)-1));
                p+=(strlen(gatewayProductClass)-1);
        }
}
...
static void read_gateway_info(void)
{
	...
	fp = fopen(DNSMASQ_GATEWAY_FILE, "r");
	if(fp){
/*NETGEAR Added Change Start*/
		my_syslog(LOG_INFO, _("TR-111: reading gateway info"));
/*NETGEAR Added Change End*/
		fgets(gatewaySerialNumber, 256, fp);
		fgets(gatewayProductClass, 256, fp);
		fclose(fp);
	}
	else{
		my_syslog(LOG_WARNING, _("TR-111: reading gateway info file open fail"));
		strcpy(gatewaySerialNumber, "");
		strcpy(gatewayProductClass, "");
	}
	return;
}
#endif
