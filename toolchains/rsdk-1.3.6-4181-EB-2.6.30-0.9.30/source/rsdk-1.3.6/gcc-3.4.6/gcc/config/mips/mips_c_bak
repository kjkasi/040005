...
mips_select_section (tree, int, unsigned HOST_WIDE_INT)
  ATTRIBUTE_UNUSED;
     ...
     static void mips_backend_reinit (void);
     static void mips_init_tables (void);
...
/* Whether we are generating mips16 hard float code.  In mips16 mode
   ...
   should arrange to call mips32 hard floating point code.  */
     ...
     static int mips_init_target_flags;
     static int mips_init_split_addresses;
     static int mips_init_schedule_insns;
     static int mips_init_align_loops;
     static int mips_init_align_jumps;
     static int mips_init_align_functions ;
     ...
     static enum function_mode func_mode = NORMAL;
...
int mips_original_mips16 = 0;
static int mips_base_arch;
const char *mips_base_arch_string = (char*)0;
...
override_options (void)
{
  ...
  mips_original_mips16 = target_flags & MASK_MIPS16; 
  target_flags ^= mips_original_mips16;
  ...
  if (mips_original_mips16 && (flag_pic || TARGET_ABICALLS)) 
    { 
      warning ("%s is not supported with -mips16, ignored", 
               (flag_pic > 1 ? "-fPIC" : flag_pic ? "-fpic" : "-mabicalls"));
      target_flags &= ~MASK_ABICALLS; 
  ...
  if (TARGET_LX5280 || TARGET_RLX4181 || TARGET_LX4180 || TARGET_RLX5181)
    {
      if (!mips_original_mips16)
        target_flags |= MASK_SOFT_FLOAT;
    }

  mips_init_target_flags = target_flags;
  mips_init_split_addresses = mips_split_addresses;
  mips_init_schedule_insns = flag_schedule_insns;
  mips_init_align_loops = align_loops;
  mips_init_align_jumps = align_jumps;
  mips_init_align_functions = align_functions;
  
  mips_base_arch = mips_arch;
  mips_base_arch_string= mips_arch_string;

  mips_set_mips16_mode (mips_original_mips16 != 0);
}

static void mips_prepare_function_start (tree fndecl)
{
  tree x;
  int mode;
  ...
  if (mips_original_mips16)
    mode = lookup_attribute("nomips16", TYPE_ATTRIBUTES(TREE_TYPE(fndecl))) ? 0 : 1;
  else
    mode = lookup_attribute("mips16", TYPE_ATTRIBUTES(TREE_TYPE(fndecl))) ? 1 : 0;

  mips_set_mips16_mode(mode);
}

int mode_switch_happen = 0;

static void mips_set_mips16_mode (int ismips16)
{
  static int first = 1;
  int mode_switch = 0; 

  if (ismips16 && func_mode == NORMAL)
    {
      mode_switch = 1;
      func_mode = MIPS16;
      ...
      if (!first)
        mode_switch_happen = 1;

      mips_arch = PROCESSOR_DEFAULT;
      mips_arch_string = "default";

      mips_tune = PROCESSOR_DEFAULT;
  ...
  else if (!ismips16 && func_mode == MIPS16)
    {
      target_flags = mips_init_target_flags;
      mode_switch = 1;
      ...
      func_mode = NORMAL;
      target_flags &= ~MASK_MIPS16;
      mips_split_addresses = mips_init_split_addresses;
      flag_schedule_insns = mips_init_schedule_insns;

      mips_arch =  mips_base_arch;
      mips_arch_string = mips_base_arch_string;
      ...
      if (mips_init_align_loops > 0)
        align_loops = mips_init_align_loops;

      if (mips_init_align_jumps > 0)
        align_jumps = mips_init_align_jumps;

      if (mips_init_align_functions > 0)
        align_functions = mips_init_align_functions;
    }
          ...
          if (mode == CCmode)
            {
              if (! ISA_HAS_8CC)
                ...
                temp = (ST_REG_P (regno) || GP_REG_P (regno) || FP_REG_P (regno));
            }
